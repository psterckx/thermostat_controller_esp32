...
#define ACTUATE_SOLENOID_TOPIC "thermostat_controller_esp32/actuate_solenoid"
#define IN_ERROR_TOPIC "thermostat_controller_esp32/error"
...
void changeTemperature(int desired_temp) {
  ...
  if (desired_temp < thermostat_temp) {
      ...
      actuateSolenoid(S1, 1200, 500);
    while (desired_temp < thermostat_temp && nActuations < MAX_ACTUATIONS) {
        actuateSolenoid(S1, 250, 250);
        nActuations += 1;
        thermostat_temp -= 1;
    }
  } else {
      Serial.println("actuating S2");
      actuateSolenoid(S2, 1200, 500);
    while (desired_temp > thermostat_temp && nActuations < MAX_ACTUATIONS) {
        actuateSolenoid(S2, 250, 250);
        nActuations += 1;
        thermostat_temp += 1;
    }
  }
  ...
  publishUpdateState();
  if (nActuations == MAX_ACTUATIONS) {
    Serial.println("ERROR");
    client.publish(IN_ERROR_TOPIC, "Error: max actuations reached. Reset required. Set temperature: " + String(thermostat_temp) + ".");
    client.disconnect();
  }
}
...
void actuateSolenoid(int id, int firstDelay, int secondDelay) {
  ...
  client.publish(ACTUATE_SOLENOID_TOPIC, "ACTUATE_SOLENOID");
